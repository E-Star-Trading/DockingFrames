...
/*
 * Bibliothek - DockingFrames Library built on Java/Swing, allows the user to "drag and drop" panels containing any
 * Swing-Component the developer likes to add. Copyright (C) 2009 Benjamin Sigg This library is free import
 * java.util.HashMap; software; you can redistribute it and/or modify it under the terms of the GNU Lesser General
 ...
 * USA Benjamin Sigg benjamin_sigg@gmx.ch CH - Switzerland
 */
...
/**
 * A panel showing some {@link bibliothek.gui.dock.action.DockAction}s. The panel can show a menu for actions which to
 * do not have enough space. Clients using this panel should call {@link #getPreferredSizes()} to get a list of possible
 * sizes of this panel, and then {@link #setVisibleActions(int)} in order to use one size. This panel will
 * {@link #revalidate()} itself whenever the number of actions changes.
 * 
 * @author Benjamin Sigg
 */
public class XButtonPanel extends ButtonPanel {
	 ...
	 */
	public XButtonPanel(boolean menu) {
		super(menu);
		setLayout(null);
	...
	/**
	 * Creates a new item for <code>action</code> which will be shown on this panel. The default implementation will use
	 * the {@link ActionViewConverter} of <code>dockable</code> to create the view. The {@link ViewTarget} is set to
	 * {@link ViewTarget#TITLE}. Subclasses may override this method to create custom views or use other settings.
	 * 
	...
	/**
	 * Gets the orientation of this panel. The orientation tells whether the buttons have to be layed out horizontally
	 * or vertically.
	 * 
	...
	/**
	 * Changes the content which is shown, ensures that the current properties are used.
	 */
	private void set() {
	 ...
	 * @throws IllegalArgumentException if <code>dockable</code> or <code>source</code> is <code>null</code> while the
	 *         other is not <code>null</code>.
	 */
	...
	/**
	 * Gets all the {@link ActionItem}s ordered by their importance, the most important item is at the front.
	 * 
	...
	/**
	 * Computes the preferred sizes of this panel. Dimension <code>result[n]</code> would be the size required if
	 * <code>n</code> actions are shown. The number <code>n</code> should be used for calling
	 * {@link #setVisibleActions(int)}.
	 * 
	...
	/**
	 * Sets the number of actions which should be shown directly on this panel. If not all available actions are shown,
	 * the remaining actions are put together in a menu.
	 * 
	...
	public void doLayout() {
		...
		if (orientation.isHorizontal()) {
			...
			for (int i = 0, n = getComponentCount(); i < n; i++) {
				...
				// @liso: to center the close button (x) in dockables
				height = height - 5;
				if (width == current.width) {
