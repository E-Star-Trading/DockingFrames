 ...
 */
public class XEclipseTabPane extends CombinedStackDockComponent<XEclipseTab, XEclipseMenu, XEclipseTabInfo> implements BorderedComponent{
   /**
    ...
    */
          ...
          if (window != newWindow && (lFlags & HierarchyEvent.PARENT_CHANGED) != 0) {
             ...
             if (newWindow != null) {
                ...
                if (getSelectedIndex() != -1) {
                   XEclipseTab tab = getTab(getSelectedDockable());
    ...
    private PropertyValue<XTabPainter> tabPainter = new PropertyValue<XTabPainter>( XEclipseTheme.TAB_PAINTER ){
        @Override
        protected void valueChanged( XTabPainter oldValue, XTabPainter newValue ){
            updateTabPainter();
        }
    };
     ...
     */
    public XEclipseTabPane( XEclipseTheme theme, DockStation station ){
        this.theme = theme;
        this.station = station;
        
        setInfoComponent( new XEclipseTabInfo( this ) );
    ...
    @Override
    protected CombinedStackDockContentPane createContentPane( CombinedStackDockComponent<XEclipseTab, XEclipseMenu, XEclipseTabInfo> self ){
        return new XEclipseTabPaneContent( (XEclipseTabPane)self );
    }
    ...
    public void setController( DockController controller ){
        ...
        for( XEclipseTab tab : getTabsList() ){
            tab.setController( controller );
        }
        for( XEclipseMenu menu : getMenuList() ){
    ...
    @Override
    public void setInfoComponent( XEclipseTabInfo info ){
        XEclipseTabInfo old = getInfoComponent();
        if( old != null ){
     ...
     */
    public XTabPainter getTabPainter(){
    ...
    protected void updateTabPainter(){
        ...
        XTabPainter painter = getTabPainter();
    ...
    @Override
    protected XEclipseTab newTab( Dockable dockable ){
        XTabComponent component = getTabPainter().createTabComponent( this, dockable );
        XEclipseTab tab = new XEclipseTab( this, dockable, component );
    ...
    @Override
    protected void tabRemoved( XEclipseTab tab ){
        tab.unbind();
    }
     ...
     */
    public int indexOfVisible( XTabComponent tab ){
        for( XEclipseTab eclipse : getTabsList() ){
    ...
    @Override
    public XEclipseMenu newMenu(){
        XEclipseMenu menu = new XEclipseMenu( this );
        menu.setController( getController() );
        return menu;
    }
    
    @Override
    protected void menuRemoved( XEclipseMenu menu ){
